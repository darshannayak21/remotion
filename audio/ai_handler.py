...
import json
import time
from system_prompt import get_conversational_prompt, get_escalation_prompt, get_live_coaching_prompt
...
class AIHandler:
    ...
    def __init__(self):
        ...
        if self.recognizer:
            self.recognizer.pause_threshold = 2.0 # Wait 2.0 seconds of silence before cutting off user
    ...
    def capture_robust_speech(self) -> str:
        ...
        try:
            with sr.Microphone(device_index=mic_index) as source:
                print("[AI] Adjusting ambient noise for 0.5s...")
                self.recognizer.adjust_for_ambient_noise(source, duration=0.5)
                
                print("[AI] Listening for your question...")
                audio = self.recognizer.listen(source, timeout=12.0, phrase_time_limit=25.0)
    ...
    def get_gemini_conversational(self, user_query: str, current_context: dict, exercise_name: str) -> str:
        ...
        system_instructions = get_conversational_prompt()
        live_context_str = f"Live Posture Telemetry:\n{json.dumps(current_context, indent=2)}\n\n"
        full_query = f"[SYSTEM INFO] Darshan's Current Assigned Target Exercise: {exercise_name.upper()}.\n\n" + live_context_str + "Darshan just told you: " + user_query
    ...
    def get_gemini_escalation(self, current_context: dict) -> str:
        ...
        system_instructions = get_escalation_prompt()
        live_context_str = f"Live Errors Detected:\n{json.dumps(current_context.get('errors', []), indent=2)}\n\n"
        full_query = live_context_str + "Darshan is struggling. Provide brief escalation guidance right now."
    ...
    def get_gemini_live_cue(self, error: dict, current_context: dict) -> str:
        ...
        system_instructions = get_live_coaching_prompt(current_context)
    ...
    def execute_conversational_loop(self, state_manager, context_getter, voice_engine, exercise_name: str) -> None:
        ...
        voice_engine.speak("Yes Darshan, how may I help you?", block=True)
