...
export default function LibraryPage() {
  ...
  const saveGroup = () => {
    ...
    localStorage.setItem("flex_workout_groups", JSON.stringify(updated));
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4">
          {savedGroups.length === 0 ? (
            <div className="text-center py-20 glass rounded-3xl mt-4">
              <p className="text-slate-500 text-sm mb-4 font-medium">You haven't created any custom routines yet.</p>
              <button
                onClick={() => {
                  ...
                  <div className="p-5 flex-1 bg-gradient-to-br from-white/60 to-transparent">
                    ...
                    <div className="mt-4 flex flex-wrap gap-1.5">
                      {group.exercises.slice(0, 4).map(exId => {
                        ...
                        return ex ? (
                          <span key={exId} className="text-[10px] font-medium bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded-md shadow-sm">
                            {ex.name}
                          </span>
                        ) : null;
                      })}
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm("Delete this routine?")) {
                          const w = savedGroups.filter(g => g.id !== group.id);
                          setSavedGroups(w);
                          localStorage.setItem("flex_workout_groups", JSON.stringify(w));
                        }
                      }}
      ...
      >
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 text-sm focus:outline-none focus:border-maroon-300 focus:ring-2 focus:ring-maroon-100 transition-all"
          />
        </div>
        ...
        <button
          ...
          className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition-all ${
            !selectedCategory
              ...
              : "bg-white border border-slate-200 text-slate-600 hover:border-maroon-200"
          }`}
        >
          All
        </button>
          ...
          {filtered.map((ex, i) => (
            <motion.div key={ex.id} custom={i} variants={fadeUp}>
              ...
              >
                ...
                {/* Image placeholder */}
                <div className="aspect-[4/3] bg-gradient-to-br from-maroon-50 to-slate-50 rounded-xl flex items-center justify-center mb-3 border border-maroon-100/30">
                  <div className="text-center">
                    <Target size={28} className="text-maroon-400 mx-auto" />
                    <p className="text-[10px] text-slate-400 mt-1">Image</p>
                  </div>
                </div>
                ...
                <h3 className="font-semibold text-sm text-slate-800 mt-2 mb-1">
                  {ex.name}
                </h3>
        ...
        <motion.div initial="hidden" animate="visible" className="space-y-2">
          {filtered.map((ex, i) => (
            <motion.div key={ex.id} custom={i} variants={fadeUp}>
              ...
              >
                {/* Small image */}
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-maroon-50 to-slate-50 flex items-center justify-center shrink-0 border border-maroon-100/30">
        ...
        {isSelectionMode && selectedExercises.length > 0 && (
          ...
          >
            <div className="bg-slate-900 border border-slate-700 shadow-2xl rounded-2xl p-2 pl-6 flex items-center gap-6">
              ...
              <button
                onClick={() => router.push(`/workout?queue=${selectedExercises.join(",")}`)}
